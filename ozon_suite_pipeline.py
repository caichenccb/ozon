...
def generate_image(prompt: str, output_path: Path, product: ProductInput) -> Path:
    image_path = Path(product.image_path) if product.image_path else None
    ...
    if image_path and image_path.exists():
        with image_path.open("rb") as image_file:
            response = client.images.edit(
                ...
                prompt=(
                    prompt
                    + "\nKeep the product identity, shape, color, material, and proportions faithful to "
                    "the supplied product image."
                ),
                size="1024x1024",
            )
    else:
