...
├─────────────────────────────────────────────┤""")
    ...
    keywords = oz.search_keywords_ru
    print(f"""
│ 4. Ozon运营适配
...
│                                             │""")
    items = [
        ("① 市场需求", s.market_demand, 25),
        ("② 利润空间", s.profit_margin, 25),
        ("③ 竞争难度", s.competition_ease, 20),
        ("④ 差异化能力", s.differentiation, 20),
        ("⑤ 转化潜力", s.conversion_potential, 10),
    ]
...
┌─ 6. 最终商业判断 ─────────────────────────┐
│                                             │
│  结论：{v.conclusion_cn:<20}            │
│                                             │
│  核心原因：                                  │
│  {v.core_reason:<47} │
│                                             │
│  是否放量：{v.scale_decision:<20}            │
│                                             │
└─────────────────────────────────────────────┘""")
...
def export_report(
    ...
    """
    导出完整报告为 JSON 文件。
    包含 Layer 1 + Layer 2 + Layer 3 全部数据。
    """
    ...
    report = {
        ...
        "layer_3_report": {
            "summary": {
                ...
                "grade_cn": result.scores.grade_cn(),
                "conclusion": result.verdict.conclusion,
                "conclusion_cn": result.verdict.conclusion_cn,
            }
        },
        "user_input": {
            "name": product.name,
    ...
    safe_name = "".join(
        ...
        for ch in (product.name or "product")
    ).strip("_") or "product"
